...
function newLotteryGame(uint128 _maxTickets, uint256 _ticketValue, uint16 _prizePercentage) public onlyOwner {
    ...
    require(_maxTickets > 0, "maxTickets can no be zero");
    require(_ticketValue > 0, "ticketValue can no be zero");
    require(_prizePercentage > 0, "prizePercentage xan no be zero");
    require(_prizePercentage <= 100, "prizePercentage can no be more than 100");
...
function setMaxTickets(uint128 _maxTickets) public onlyOwner {
    require(_maxTickets > 0, "Tickets number can no be zero");
    maxTickets = _maxTickets;
}

// set the unitary ticket price.
function setTicketPrice(uint256 _ticketValue) public onlyOwner {
    require(_ticketValue > 0, "Tickets price an no be zero");
...
function setPrizePercentage(uint16 _prizePercentage) public onlyOwner {
  require(_prizePercentage > 0, "Prize can no be zero");
  require(_prizePercentage <= 100, "Prize can no be more than 100");
